...
def moveb_m(state,goal):
    ...
    waiting_blocks = [b for b in all_blocks(state) if status(b, state, goal) == 'waiting']
    if waiting_blocks != []:
        return [[('move_one', b, 'table'), ('move_blocks', goal)] for b in waiting_blocks]
...
"""
pyhop2.declare_methods('move_blocks',moveb_m)
...
pyhop2.declare_methods('move_one',move1)
...
pyhop2.declare_methods('get',get_m)
...
pyhop2.declare_methods('put',put_m)
